...
class Database_Handler():
    ...
    def get_identity_by_role(self, role_name, guild_id) -> Identity:
        cursor.execute(f'SELECT display_name, lexicon, avatar, allowed_words, display_name_with_id, override, override_chance FROM identities WHERE name = "{role_name}" AND guild_id = {guild_id}')
        data = cursor.fetchall()
